...
function resolveKbRegistryLoader() {
  ...
  return {
    loadKbRegistry: () => ({
      knowledge_bases: {
        gt: { id: process.env.TOULMIN_GT_KB_ID || process.env.WOODPECKER_GT_KB_ID || '', name: '技术与工程教学' },
        it: { id: process.env.TOULMIN_IT_KB_ID || process.env.WOODPECKER_IT_KB_ID || '', name: '信息科技教学' }
      },
